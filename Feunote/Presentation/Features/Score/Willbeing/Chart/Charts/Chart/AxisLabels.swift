...
public struct AxisLabels<Content: View>: View {
    ...
    public init<Data, Label>(_ axis: Axis = .horizontal, data: Data, @ViewBuilder label: @escaping (Data.Element) -> Label) where Content == ForEach<Data, Data.Element.ID, Label>, Data: RandomAccessCollection, Label: View, Data.Element: Identifiable {
        self.axis = axis
        labels = data.map { AnyView(label($0)) }
    }

    public init<Data, ID, Label>(_ axis: Axis = .horizontal, data: Data, id _: KeyPath<Data.Element, ID>, @ViewBuilder label: @escaping (Data.Element) -> Label) where Content == ForEach<Data, ID, Label>, Data: RandomAccessCollection, ID: Hashable, Label: View {
        self.axis = axis
        labels = data.map { AnyView(label($0)) }
    }
