...
public struct ColumnChartStyle<Column: View>: ChartStyle {
    ...
    private func offset(for element: ColumnData, geometry: GeometryProxy, columnWidth _: CGFloat, dataCount _: Int, hasNegativeValues: Bool) -> CGSize {
        let x: CGFloat = 0
        var y: CGFloat = 0
        let height = columnHeight(data: element.data, in: geometry.size.height, hasNegativeValues: hasNegativeValues)
    ...
    private func bottomAlignmentGuide(dimension _: ViewDimensions, for data: CGFloat, in availableHeight: CGFloat, hasNegativeValues: Bool) -> CGFloat {
        let height = columnHeight(data: data, in: availableHeight, hasNegativeValues: hasNegativeValues)
        if data < 0 {
